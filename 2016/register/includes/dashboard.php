<?php
	//die("<h1 style='position:absolute;left:500px;'>PAGE UNDER CONSTRUCTION</h1>");
	...
	$eventlist = getRegisteredEvents1($userid);
	for ($i = 0; $i < count($eventlist); $i++) {
		$eventname = getEventbyid($eventlist[$i]['event']);
		$registered .= "<tr>
			   <td><p style='padding:3px; line-height:20px;'>{$eventname[0]['event_name']}</p></td></tr>";/*
		...
		for ($j = 0; $j < count($events); $j++) {
			if ($eventcat == $events[$j]['event_category_id']) {
				if (strtotime($events[$j]['start']) <= time() && time() < strtotime($events[$j]['end'])) {
					$ongoing .= "<tr><td>{$events[$j]['event_name']}</td><td>".ucFirst($categories[$i]['name'])."</td></tr>";/*.format_date($events[$j]['end'])."</td></tr>";*/
				}
			}
		}
	}
	...
	for ($i = 0; $i < count($upcmevents); $i++) {
		if ($count < 5 && strtotime($upcmevents[$i]['start']) > strtotime(date('Y-m-d H:i:s'))) {
			...
			$upcoming .= "<tr><td>{$upcmevents[$i]['event_name']}</td><td>".ucFirst($categories[0]['name'])."</td></tr>";
		/*<td>".format_date($upcmevents[$i]['start'])."</td></tr>";*/
		}
	}	
?>
...
<div id="container">
  <div class="shell">
    ...
    <!-- Main -->
      ...
      <!-- Content -->
      <div id="content" style="width:42%; margin-left: 40%; margin-top: 10px;">
        <!-- Box -->
       <?php
			if ($registered) {
				echo "<div class='box'>
      ...
      <!-- End Content -->
      <!-- Sidebar -->
      <div id="sidebar" style="width:54%;padding-right: 10%; margin-top: 20px;">
        <!-- Box -->
        <div class="box" style="height:827px;">
          <!-- Box Head -->
          ...
          <div class="table" >
		<p style="font-size:30px;text-align:center;margin-top:70px;line-height:1;">The Much Awaited Effervescence'15 has now BEGUN!!! .</p>
				<?php
					$news = getNews();
