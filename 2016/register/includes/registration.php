		...
		if(!alreadyRegistered1($userid, ''.$regval)) {
			if(isProfileComplete($user)) {
				...
				if($reg->addPart($regval, $userid)){
					$registerscs = "<p style='color:#74a446; font-size:15px;margin-left:200px;' > You have successfully registered to the Event! </p>";
				}else {
					$registerscs = "<p style='color:#74a446; font-size:15px;margin-left:200px;' > Sorry!! You COuldnote be registered</p>";
				}
			} else {
				$registerscs = "<p style='color:red; font-size:15px;margin-left:200px;' > First you need to complete your profile information to apply for Events.<p>";
			}
		} else {
			$registerscs = "<p style='color:#74a446; font-size:15px;margin-left:200px;' > You are already registered to the Event! </p>";
		}
					...
					if ($count[$i]) {
						...
						for ($j = 1; $j <= count($events); $j++) {
							...
							if ($events[$j-1]['event_category_id'] == $eventcat) {
								...
								} else if($events[$j - 1]['event_name'] == "Incendiary") {
									echo "<a href='http://goo.gl/forms/J0qUtNQTCw'>Register</a>";
								}
								  else {
									echo"
