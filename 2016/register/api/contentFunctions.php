...
// 	      }
	...
	function HITNFormValidate ($githubId, $accomodation, $userexp, $resume, $mhl_username, $mhl_email) 
	{
	      $msg = "";
// 	      if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
// 		   $msg = " Please Enter a valid email id.";
// 	      }
	      ...
	      if (existAccount($githubId)) {
		   $msg .= "Github ID already in use.";
	      } 
	...
	function existSpaceHurdleUser($username) //Check if a username already exists
	{	
		$sql = "SELECT * FROM space_hurdle_users WHERE username = :username AND isdeleted = 0";
		$params = array(':username' => $username);
		$result = DatabaseHandler::GetAll($sql, $params);
		if(count($result) == 1)
			return 1;
	...
	function addSpaceHurdleUser($name, $phone, $score) //Create a new account
	{
				
		//Add an account
		$sql = "INSERT INTO space_hurdle_users (username, phone, score, added_on) VALUES(:username, :phone, :score, NOW())";
		$params = array(':username' => $name, ':phone' => $phone, ':score' => $score);
		$result = DatabaseHandler::Execute($sql, $params);		
			
		return;
	}

	function updateSpaceHurdleUser($name, $phone, $score) //Create a new account
	{
				
		//Update an account
		$sql = "UPDATE space_hurdle_users SET score = :score WHERE username = :username AND isdeleted = 0";
		$params = array(':username' => $name, ':score' => $score);
		$result = DatabaseHandler::Execute($sql, $params);		
			
		return;
	}

	function updateHITN($githubId, $accomodation, $userexp, $resume, $mhl_username, $mhl_email) //Create a new account
	{
				
		//Add an account
		$sql = "INSERT INTO hitn_users (github_id, accomodation, userexp, resume, added_on, mhl_username, mhl_email) VALUES(:github_id, :accomodation, :userexp, :resume, NOW(), :mhl_username, :mhl_email)";
		$params = array(':github_id' => $githubId, ':accomodation' => $accomodation, ':userexp' => $userexp, ':resume' => $resume, ':mhl_username' => $mhl_username, ':mhl_email' => $mhl_email);
		$result = DatabaseHandler::Execute($sql, $params);		
	...
	function getScore($username)  // Returns account information
	{
		$sql = "SELECT * FROM space_hurdle_users WHERE username = :username AND isdeleted = 0";
		$params = array(':username' => $username);
		$result = DatabaseHandler::GetAll($sql, $params);
	...
	function saveProfile($user_id, $name, $guvera_id, $college_id, $profession, $gender, $accommodation, $address) //Update any changes to profile
	{
		$x = 10000 + $user_id;
		$reg_id = "APRO16" . $x;
		$lock = 1;
		$sql = "UPDATE user_details SET name = :name, college_id = :college_id, profession = :profession, gender = :gender, modified_on = NOW(), accommodation = :accommodation, address = :address, reg_id = :reg_id, locked = :lock, guvera_id = :guvera_id WHERE user_id = :user_id AND isdeleted = 0";
		$params = array(':user_id' => $user_id, ':name' => $name, 'college_id' => $college_id, ':profession' => $profession, ':gender' => $gender,':accommodation' => $accommodation, ':address' => $address, ':reg_id' => $reg_id, ':lock' => $lock, ':guvera_id' => $guvera_id);
		$result = DatabaseHandler::Execute($sql, $params);
	}
	...
	function alreadyRegistered1($uid, $event)
	{
		$enc = new ENC_REG();
	...
	function getRegisteredEvents1($userid)  // Returns list of registered events of the user
	{
		$sql = "SELECT * FROM event_part WHERE uid = :userid";
		$params = array(':userid' => $userid);
		$result = DatabaseHandler::GetAll($sql, $params);
		return $result;
	}
