<?php
    ...
    class RANK extends ENC {
		...
		private function generateRankList() {
		    ...
		    $query = "SELECT @row_num := @row_num + 1 AS rank, tmp.username FROM ( SELECT `username`, count(*) AS NS, max(TIMESTAMP(`time`)) AS TIME
			    FROM `" . $this->event . "_submissions` WHERE `valid` = 0 GROUP BY `username` ORDER BY NS DESC, TIME ASC) AS tmp, (SELECT @row_num := 0) r";
		...
		private function generateMyRank() {
		    ...
		    $query = "SELECT * FROM ( SELECT @row_num := @row_num + 1 AS rank, tmp.username FROM ( SELECT `username`, count(*) AS NS, max(TIMESTAMP(`time`)) AS TIME
			    FROM `" . $this->event . "_submissions` WHERE `valid` = 0 GROUP BY `username` ORDER BY NS DESC, TIME ASC) AS tmp, (SELECT @row_num := 0) r ) AS t WHERE t.username = :uid";
		...
		private function generateRankList1() {
		    if($this->event == null || $this->seed == null || $this->uid == null || $this->dbh == null) return null;
		...
		private function generateMyRank1() {
		    if($this->event == null || $this->seed == null || $this->uid == null || $this->dbh == null) return null;
