...
if($score!=290 && $qs_ans!=29)
{
  header("Location: play.php");
}

$ques=mysql_query("SELECT * FROM ques WHERE ques_id=30");
...
if(isset($_POST['q30_submit']))
{
	...
	{
		$q30_score=10;
		if(mysql_query("UPDATE users SET score=300,qs_ans=30 WHERE user_id=".$_SESSION['user']))
    		{
      			?>
      			<script>alert('you Have Completed The Quest');</script>
      			<?php
			header("Location: play.php");
    		}
		else
...
<header>
  ...
  <div id="test1" class="col s12 m9"><br>
        ...
        <img id = "mainimg" class="materialboxed initialized" height="400" width="650" src="images/q97235.jpg" ><br>
	       
	<div class="row">
    <form method="post" class="col s12 ">
      <div class="row">
        <div class="input-field  col  s12 m6" id = "mainfield">
          <input placeholder="Enter text" id="q30_ans" name="q30_ans" type="text" class="validate white-text">
          <label for="q30_ans">Answer</label>
        </div>
 <button class="btn waves-effect waves-light btn-large black" type="submit" name="q30_submit" id ="mainbutton" ><i class="material-icons right">send</i>
  </button>
  </div>
    </form>
  </div></div>
