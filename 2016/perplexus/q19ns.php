...
if($score!=180 && $qs_ans!=18)
{
  header("Location: play.php");
}


$ques=mysql_query("SELECT * FROM ques WHERE ques_id=19");
...
if(isset($_POST['q19_submit']))
{
	...
	{
		$q19_score=180;
		if(mysql_query("UPDATE users SET score=190,qs_ans=19 WHERE user_id=".$_SESSION['user']))
    		{
      			?>
...
<header>
  ...
  <div id="test1" class="col s12 m9"><br>
        ...
        <img id = "mainimg" class="materialboxed initialized" height="400" width="650" src="images/q36691.jpg" ><br>
	       
	<div class="row">
    <form method="post" class="col s12 ">
      <div class="row">
        <div class="input-field  col  s12 m6" id = "mainfield">
          <input placeholder="Enter text" id="q19_ans" name="q19_ans" type="text" class="validate white-text">
          <label for="q19_ans">Answer</label>
        </div>
 <button class="btn waves-effect waves-light btn-large black" type="submit" name="q19_submit" id ="mainbutton" ><i class="material-icons right">send</i>
  </button>
  </div>
    </form>
  </div></div>
