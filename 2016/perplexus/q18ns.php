...
if($score!=170 && $qs_ans!=17)
{
  header("Location: play.php");
}


$ques=mysql_query("SELECT * FROM ques WHERE ques_id=18");
$quesRow=mysql_fetch_array($ques);
$q18=$quesRow['q_ans'];

if(isset($_POST['q18_submit']))
{
	$q18_ans = mysql_real_escape_string($_POST['q18_ans']);
	$q18_ans=trim($q18_ans);
	$q18_ans=md5($q18_ans);
	$q18_ans=trim($q18_ans);
	...
	if($q18_ans==$q18)
	{
		...
		if(mysql_query("UPDATE users SET score=180, qs_ans=18 WHERE user_id=".$_SESSION['user']))
    		{
      			?>
...
<header>
  ...
  <div id="test1" class="col s12 m9"><br>
        ...
        <img id = "mainimg" class="materialboxed initialized" height="400" width="650" src="images/q92825.png" ><br>
	       
	<div class="row">
    <form method="post" class="col s12 ">
      <div class="row">
        <div class="input-field  col  s12 m6" id = "mainfield">
          <input placeholder="Enter text" id="q18_ans" name="q18_ans" type="text" class="validate white-text">
          <label for="q18_ans">Answer</label>
        </div>
 <button class="btn waves-effect waves-light btn-large black" type="submit" name="q18_submit" id ="mainbutton" ><i class="material-icons right">send</i>
  </button>
  </div>
    </form>
  </div></div>
