...
scroll_down.init = function(){
	$('#world > section').not('.home').each(function(){
		var s = ( $(this).hasClass('events') ) ? $(this).find('> section.container') : $(this);
		$('<a class="scroll_down fa fa-chevron-down"></a>').appendTo(s);
	});
}
scroll_down.manage = function(){
	var section = ( $('#world > section.selected').hasClass('events') ) ? $('#world > section.selected > section.container') : $('#world > section.selected');
...
cuul.init = function(){
	...
	$('.about .wrapper > h3, .skills > .wrapper > h3, .events > .container > a, .contact .social li').addClass('cuul scaleIn');
	$('.events > .container > h3, .events > .container > img, .events > .container > ul').addClass('cuul fadeInDown');
		
	$('#world > section, #world > section.events > .container').on('scroll', function(e){ cuul.scroll(e) });
...
newhash.change = function(first_exec){
	...
	if(first_exec){
		...
		pageSlider.move();
		if(newhash.events) events.change();
	}
	//particles.hashchange();
	language();
}
...
pageSlider.init = function(){
	...
	scroll_down.manage(); 
	if(ss.hasClass('events')) events.initSection();
...
pageSlider.move = function(){
	...
	if( destination != current ){
		...
		setTimeout(function(){ 
			...
			scroll_down.manage(); 
			if(newhash.page == 'events') events.initSection();
		}, tempo);	
...
events.init = function(){
	this.container = $('#world > section.events section.container');
	this.link = $('#world > section.events aside ul li');
	this.h3s = [];
	var that = this;
	...
	$(window).resize( function(){ if(newhash.page == 'events') that.initSection() })
	
}
events.initSection = function(){
	var that = this;
	this.h3s = [];
	var scroll = that.container.scrollTop();
...
events.change = function(){
	var w = $('h3.'+newhash.events, this.container).position().top;
